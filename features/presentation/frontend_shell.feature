Feature: Kivy frontend shell for multi-framework localization workflows
  ...
  I want a navigable frontend shell backed by contracts and test doubles
  ...
  Scenario: Trigger a sync action from the project detail
    Given the frontend shell is wired with seeded frontend test doubles
  ...
  Scenario: Trigger an audit action from the project detail
    Given the frontend shell is wired with seeded frontend test doubles
    And the operator has opened the detail for project "wp-site"
  ...
  Scenario: Trigger PO processing from the project detail
    Given the frontend shell is wired with seeded frontend test doubles
    And the operator has opened the detail for project "wp-site"
