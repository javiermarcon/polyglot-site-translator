...
- the project detail screen exposes that workflow as a generic `Translate` action, and the pre-run popup can override locales, `.mo` compilation, external-translator usage, translation-cache usage, `only_fuzzy`, `dry-run`, `stats-only`, and inconsistency reporting for the current execution
- the visible translation summary now mirrors the legacy `ProcessStats` granularity, including found vs processed families, fuzzy counts, reused-from-other-variant counts, sync-only skips, and variant-difference details
