...
class SqliteSyncScopeRepository:
    ...
    def load_sync_scope_settings(
        ...
        try:
            with self._connect() as connection:
                global_rules = _fetch_configured_rules(
                    connection, "sync_scope_global_rules"
                )
            ...
            msg = (
                "SQLite sync scope persistence read failed at "
    ...
    def save_sync_scope_settings(
        self, sync_scope_settings: AdapterSyncScopeSettings
    ) -> None:
        ...
        try:
            with self._connect() as connection:
                ...
                _insert_configured_global_rules(
                    connection, sync_scope_settings.global_rules
                )
            ...
            msg = (
                "SQLite sync scope persistence write failed at "
                f"{self._location.database_path}."
            )
            raise SyncScopePersistenceError(msg) from error
    ...
    def _ensure_schema(self) -> None:
        ...
        try:
            ...
            msg = (
                "SQLite sync scope schema initialization failed at "
                f"{self._location.database_path}."
            )
            raise SyncScopePersistenceError(msg) from error
        except OSError as error:
            msg = (
                "SQLite sync scope directory could not be prepared at "
                f"{self._location.directory}."
            )
            raise SyncScopePersistenceError(msg) from error
...
class ConfiguredSqliteSyncScopeRepository:
    ...
    def load_sync_scope_settings(
        ...
        default_settings = (
            default_settings or settings_state.app_settings.sync_scope_settings
        )
...
def _fetch_framework_rule_sets(
    connection: sqlite3.Connection,
...
def _configuration_exists(connection: sqlite3.Connection) -> bool:
    ...
    """
    row = connection.execute(
        "SELECT 1 FROM sync_scope_configuration LIMIT 1"
    ).fetchone()
